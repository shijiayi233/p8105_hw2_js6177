...
I read the data in file `pols-month.csv`, clean the variable name,
separate variable `mon` into integer variables `year`, `month` and
`day`. I then replace month number with month abbreviation and convert
it to lower case. I create a `president` variable taking values `gop`
and `dem`, and remove `prez_dem`, `prez_gop` and `day` variables.
...
I read the data in file `snp.csv`, clean the variable name, separate
variable `date` into integer variables `year`, `month` and `day`. I then
arrange the dataset according to `year` and `month`, and next replace
month number with month abbreviation and convert it to lower case.
Finally, I organize the dataset so that `year` and `month` are the
leading columns.
...
I swich dataset from wide to long format using `pivot_longer()`,
creating two new variables: `month` taking the value of month
abbreviation in lower case (the original column names) and
