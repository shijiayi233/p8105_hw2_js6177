...
include dumpster-specific data, round the number of sports balls to the
nearest integer and converts the result to an integer variable. As part
of data import, I specify cell range for reading so as to omit non-data
entries.
...
arrange the dataset according to `year` and `month` in ascending order.
Next,I replace month number with month abbreviation and convert it to
lower case. Finally, I organize the dataset so that `year` and `month`
