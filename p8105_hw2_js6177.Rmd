...
I read the data in file `pols-month.csv`, update the variable name, separate variable `mon` into integer variables `year`, `month` and `day`, replace month number with month abbreviation and convert it to lower case. I create a `president` variable taking values `gop` and `dem`, and remove `prez_dem`, `prez_gop` and `day` variables.
...
I read the data in file `snp.csv`, update the variable name, separate variable `date` into integer variables `year`, `month` and `day`, replace month number with month abbreviation and convert it to lower case.. I then arrange the dataset according to `year` and `month` in ascending order. Finally, I organize the dataset so that `year` and `month` are the leading columns.
